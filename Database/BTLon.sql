...
--
-- Host: localhost
-- Generation Time: Nov 30, 2015 at 02:56 AM
...
-- Triggers `Add_Book`
--
DROP TRIGGER IF EXISTS `trig_Add_Book_insert`;
DELIMITER //
CREATE TRIGGER `trig_Add_Book_insert` BEFORE INSERT ON `Add_Book`
 FOR EACH ROW BEGIN  IF NEW.Quantity_Add <0 THEN SET NEW.Quantity_Add=0; END IF;  END
//
DELIMITER ;
...
(13, 'Deutsch Wodoku: Vui Học Từ Vựng Với Ô Chữ Sudoku\r\n', 'Elke Huppertz', 198000, 7, 'Từ một khí của trời đất mà sinh. Mỗi vật riêng được một khí thiên lệch, còn người thì được trọn vẹn khí của trời đất vậy.Một khi khí trong thân người chênh lệch thì sinh ra bệnh tật, thì lại mượn món thuốc thiên lệch một khí để điều chỉnh sự nghiêng lệch trong thân ta cho trở về với quân bình thì hết bệnh vậy. Vả chăng mượn âm dương của vật để biến hóa âm dương trong thân người, nên căn cứ vào đó Thần Nông dùng thuốc để chữa '),
...
(1, 195842465, 123123, 10, '2015-11-04'),
(2, 195875465, 232323, 1, '2015-11-04'),
(3, 195875656, 23432423, 0, '2015-11-11');
...
ALTER TABLE `Add_Book`
  ADD CONSTRAINT `Add_Book_ibfk_1` FOREIGN KEY (`BID`) REFERENCES `Book` (`BID`);
